...
(function () {
  ...
  const PROJECTS = {
    ...
    SilgamSujevi: {
      ...
      year: '2026',
      desc1: '실감수제비는 제주 바닷가에서 가족과 함께했던 물수제비의 기억을 MediaPipe 동작 인식과 실시간 3D 환경으로 재현한 Unity 6 기반의 체험형 게임이다. 사용자는 카메라 앞에서 양팔을 T자로 벌려 준비한 뒤 한쪽 팔을 옆으로 크게 스윙하며 직접 물수제비를 던진다.',
      desc2: '캐릭터 에셋은 Varco 3D로 제작하고, 배경음악은 Suno AI를 활용해 구성하였다. Codex를 활용해 게임의 입력·상태·물리·점수 시스템을 개발했으며, 실제 팔 동작이 돌의 발사 방향과 속도, 회전, 물 위 바운스 횟수로 이어지도록 설계하였다.',
      sections: [
        { label: 'Concept', body: '어릴 때 제주 바닷가에서 돌을 고르고 물수제비를 던지던 기억을, 관객이 화면으로 바라보는 콘텐츠가 아니라 직접 몸을 움직여 다시 경험하는 놀이로 확장하고자 하였다. 물수제비라는 단순한 행위에 캐릭터, 바다, 물보라, 점수와 재도전의 흐름을 더해 누구나 짧은 시간 안에 이해하고 참여할 수 있는 실감형 체험을 구성하였다.' },
        { label: 'Process', body: 'Varco 3D를 활용해 게임의 주인공 캐릭터 에셋을 제작하고, Unity 6에서 제주 바닷가 환경과 물 셰이더, 물수제비 돌, 장식 오브젝트를 하나의 플레이 공간으로 구성하였다. 사용자의 움직임은 Python 기반 MediaPipe 포즈 브리지에서 어깨·팔꿈치·손목의 위치를 추적하고, T자 준비 자세와 옆 방향 스윙을 판정해 localhost UDP로 Unity에 전달한다. Unity에서는 수신된 동작 정보를 캐릭터 팔 리깅과 돌 발사 입력에 연결하고, Codex를 활용해 게임 시작·플레이·결과·재도전 상태와 로컬 TOP 10 리더보드를 개발하였다. 돌의 속도, 물 진입 각도, 공격 각도, 회전량, 수면 충돌 시 양력을 계산해 바운스를 결정하도록 구현했으며, Suno AI로 제작한 배경음악과 바다 ambience, 투척·물보라·성공 효과음을 결합해 체험의 몰입감을 높였다.' },
        { label: 'Outcome', body: '최종적으로 사용자의 실제 팔 동작을 통해 캐릭터가 반응하고, 돌이 물 위를 튕기며, 결과가 점수와 리더보드로 이어지는 카메라 기반 인터랙티브 게임 프로토타입을 완성하였다. 준비 자세와 스윙 동작을 분리해 의도하지 않은 발사를 줄이고, 물리 조건에 따라 매번 다른 물수제비 결과가 나오도록 구성했다. 이를 통해 생성형 3D 에셋과 AI 음악을 Unity 게임 개발 및 컴퓨터 비전 입력과 결합하는 제작 과정을 경험하였다.' }
      ],
      details: [
        { k: '프로젝트 유형', v: 'AI+XR 체험형 게임' },
        { k: '사용 도구', v: 'Unity 6 · MediaPipe · OpenCV · Varco 3D · Suno AI · Codex' },
    ...
    GetTheOceanSummerReady: {
      title: 'GET THE OCEAN SUMMER READY',
      cat: '부산국제마케팅광고제 YOUNG STARS 경진대회',
      year: '2026',
      desc1: 'GET THE OCEAN SUMMER READY는 매년 여름을 준비하는 사람들의 익숙한 행동을 바탕으로, 바다도 우리를 맞이할 준비가 필요하다는 질문을 던지는 캠페인이다. 운동과 산책을 해양 보호를 위한 일상의 행동으로 연결해, 거대하고 멀게 느껴지는 환경 문제를 누구나 반복할 수 있는 움직임으로 바꾸고자 하였다.',
      desc2: '캠페인의 핵심 행동은 짧은 자동차 이동을 걷기로 바꾸는 WALKOUT이다. 참가자는 돕고 싶은 해변을 선택하고, 자신의 걷기 거리를 해변의 공동 목표에 더한다. 걷기가 실제로 자동차나 택시 이동을 대체한 경우에만 타이어 마모 감소량을 추정해 기록함으로써, 상징적인 참여와 환경 효과를 구분하도록 설계하였다.',
      sections: [
        { label: 'Concept', body: '사람들은 여름과 바다를 위해 몇 달 동안 몸을 준비하지만, 바다가 우리를 맞이할 준비가 되었는지는 자주 묻지 않는다. 이 캠페인은 이 익숙한 여름 준비의 방향을 바꾸어, 우리가 걷는 시간이 바다를 준비하는 시간이 될 수 있다는 발상에서 출발하였다. WORK OUT을 WALK OUT으로 전환하고, 걷기를 특별한 장비나 전문 지식 없이 누구나 반복할 수 있는 환경 행동으로 제안하였다.' },
        { label: 'Process', body: '일상의 걷기를 공동의 해변 목표로 연결하기 위해 참가자가 먼저 돕고 싶은 해변을 선택하도록 구성하였다. 각 해변의 이전 공식 연간 방문객 수를 Summer Distance로 설정하고, “1 visitor = 1 kilometer for the ocean”이라는 규칙에 따라 방문객 한 명을 바다를 위한 1km의 상징적 목표로 환산하였다. 참가자는 QR 코드가 있는 보도 마커나 캠페인 페이지를 통해 WALKOUT을 시작하고, 걷기 거리와 연속 기록을 쌓는다. 짧은 자동차 이동을 실제로 대체했다고 확인한 거리에는 타이어 마모 감소 추정치를 별도로 적용한다. 이때 모든 걸음에 환경 효과를 곱하지 않고, 자동차 이동을 대체한 거리만 계산해 캠페인의 신뢰성을 지키도록 설계하였다.' },
        { label: 'Outcome', body: '개인의 산책은 해변별 실시간 게이지와 누적 거리, 참여자 수로 시각화되고, 목표가 100%에 도달하면 해당 해변이 SUMMER READY가 된다. 이는 해변이 과학적으로 완전히 깨끗해졌다는 주장이 아니라, 사람들이 함께 정한 상징적 행동 목표를 달성했다는 캠페인 성취를 의미한다. 보드와 영상에서는 보도 위 거리 마커, QR 코드, 공유 목표, 해변별 진행률을 통해 작은 이동이 함께 만드는 집단적 변화를 보여주었다.' }
      ],
      details: [
        { k: '프로젝트 유형', v: '광고 캠페인 기획' },
        { k: '참가 대회', v: '부산국제마케팅광고제 YOUNG STARS' },
        { k: '역할', v: '캠페인 기획 · 크리에이티브 전략 · 보드 제작' },
      ],
      media: [
    ...
    BioAx: {
          title: '연결', // 🔧 최상단 화면에 크게 표시되는 대제목입니다
          cat: '제주 바이오 AX 해커톤', // 🔧 제목 위나 옆에 달리는 카테고리 태그입니다
          year: '2026', // 🔧 연도 태그
          desc1: '연결은 제주에서 특별한 로컬 경험을 찾는 관광객을 위해, 흩어져 있는 바이오 관련 장소와 체험을 하나의 여행 코스로 이어주는 관광 서비스이다. 감귤, 동백, 용암해수, 발효식품처럼 이미 제주 여행 속에 존재하는 바이오 자원을 하나의 테마로 묶고, 관광객의 위치와 시간, 관심사에 따라 개인화된 코스를 추천하도록 설계하였다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '서비스는 AI 바이오 코스 추천, 이동 경로를 그림으로 완성하는 드로잉 코스, 방문 횟수에 따라 혜택이 확장되는 단계형 바우처로 구성된다. 이를 통해 관광객에게는 새로운 로컬 여행 경험을 제공하고, 바이오 기업과 제주도에는 방문·완주·구매 행동을 파악할 수 있는 데이터 구조를 제안하였다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
            { label: 'Concept', body: '프로젝트는 “제주에는 바이오 자원이 많지만, 왜 관광객은 제주 바이오를 여행의 경험으로 인식하지 못할까?”라는 질문에서 시작하였다. 공공데이터를 분석한 결과, 바이오 관련 장소의 관광 주목도는 인기 관광지에 비해 낮았지만, 인기 관광지의 73% 주변에는 이미 바이오 관련 POI가 존재하고 있었다. 즉, 문제는 바이오 경험이 없는 것이 아니라 감귤밭, 해녀 문화, 동백, 발효식품 같은 경험이 농업·먹거리·자연 체험으로 흩어져 있어 하나의 바이오 경험으로 연결되지 않는 데 있었다. 이에 관광객, 바이오 기업, 제주도 세 주체의 문제를 함께 해결하는 방향으로 서비스를 설계하였다. 관광객에게는 직접 찾기 어려운 바이오 장소를 짧고 재미있는 코스로 제공하고, 기업에는 어떤 원료와 체험이 실제 방문과 구매로 이어지는지 확인할 수 있는 소비자 데이터를 제공한다. 제주도와 지원기관에는 관광과 바이오 산업 지원이 실제 행동 성과로 이어졌는지 확인할 수 있는 지표를 제공한다. 즉 연결의 핵심은 단순히 장소를 추천하는 것이 아니라, 흩어진 경험을 하나의 여행으로 묶고 그 여행의 결과를 소비와 산업 데이터로 다시 연결하는 데 있다.' },
            { label: 'Process', body: '본 프로젝트에서는 UI/UX 디자인과 사용자 흐름 설계, 발표자료 제작을 담당하였다. 서비스의 주요 사용자인 관광객이 ‘바이오’라는 다소 낯선 산업 용어를 어렵게 느끼지 않도록, 산업 정보를 먼저 설명하기보다 여행 과정 속에서 자연스럽게 경험하도록 만드는 데 중점을 두었다. 먼저 사용자의 위치, 체류시간, 관심사를 입력받은 뒤 AI가 적합한 바이오 POI를 추천하고, 추천된 장소들을 하나의 완주 가능한 코스로 제시하는 흐름을 설계하였다. 이후 사용자가 이동하는 경로가 감귤이나 바이오 원료의 형태를 그리는 드로잉 코스로 완성되도록 하여, 단순한 장소 방문을 기록하고 공유할 수 있는 여행 결과물로 전환하였다. 각 지점에는 짧은 스토리 미션을 배치하고, 방문 수가 늘어날수록 제품과 체험 혜택이 확장되는 단계형 바우처를 연결해 실제 소비 전환까지 이어지도록 구성하였다. 서비스의 기반 데이터는 제주 POI 공공데이터, VisitJeju 관광정보, 제주 용암해수 산업단지 데이터 등을 활용해 구축하였다. 원천 데이터의 장소와 좌표를 정규화하고, 바이오 테마와 스토리 요소를 추출한 뒤 직접 검증한 현장 정보로 보완하여 실제 서비스에서 활용 가능한 코스 데이터로 재구성하였다. 또한 여행자 페르소나 데이터와 POI 특성을 기반으로 CatBoost Regressor를 활용해 사용자별 예상 만족도를 예측하고, 추천 POI를 드로잉 코스와 미션, 바우처로 연결하는 구조를 설계하였다. UI/UX에서는 산업 정보를 전면에 노출하기보다 관광객이 코스를 선택하고, 이동하고, 기록하고, 혜택을 받는 흐름 안에서 자연스럽게 바이오 스토리를 접하도록 구성하였다. 이를 통해 데이터 분석과 AI 추천 결과가 실제 사용자 경험으로 이어질 수 있도록 전체 서비스 구조를 시각화하고 MVP 화면으로 구현하였다.' },
            { label: 'Outcome', body: '최종적으로 홈, 여행 조건 입력, AI 코스 추천, 지도 기반 투어, 드로잉 코스와 바우처로 이어지는 모바일 서비스 MVP를 완성하였다. 관광객이 자신의 조건에 맞는 코스를 추천받고, 제주의 바이오 관련 장소를 이야기와 경로로 경험하며, 방문과 완주 결과를 기록하도록 구성하였다. 또한 코스 선택, POI 방문, 완주, 쿠폰 사용 데이터를 기업과 운영자가 확인할 수 있는 대시보드 구조를 제안하였다. 이 프로젝트를 통해 공공데이터에서 발견한 문제를 사용자 경험과 서비스 구조로 전환하고, 사용자의 행동을 기업과 지역이 활용할 수 있는 데이터로 연결하는 다면적 서비스 설계 과정을 경험하였다.' }
          ],
          details: [
            { k: '프로젝트 유형', v: '해커톤' },
            { k: '사용 툴', v: 'HTML, CSS, JS, Figma, Python' },
            { k: '역할', v: '프로젝트 기획 및 프론트엔드 디자인' },
          ],
          media: [
            { type: 'img', src: 'img/BioAx.png', label: 'img' },
            { type: 'pdf', src: 'https://drive.google.com/file/d/1dON_sgq-M_Zcy-p-uWuCNCoR3T4Pb9TW/view?usp=sharing', label: 'pdf' },
            { type: 'video', src: 'https://drive.google.com/file/d/1e_5eY_QjNEegtY20EaZWKwv3mqtSHQct/view?usp=sharing', label: 'video' },
            // { type: 'video', src: 'video/sample.mp4', label: '비디오 캡션' } // 영상을 넣고 싶으시면 이렇게 하세요!
          ]
        },
        Neuroscape: {
          title: 'Neuroscape',
          cat: '2026 BCM Academy AI X Media Startup Project',
          year: '2026',
          desc1: 'Neuroscape는 교사의 짧은 공강과 휴식 시간을 실제 회복 경험으로 전환하기 위해 개발한 EEG 기반 VR 명상 콘텐츠이다. 사용자가 VR 기기를 착용하면 외부의 시각·청각 자극에서 잠시 분리되고, 실시간으로 측정된 뇌파 변화가 미디어아트의 움직임과 형태에 반영되어 개인마다 다른 몰입 환경을 경험할 수 있도록 설계하였다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '단순히 정해진 명상 영상을 보여주는 방식이 아니라, 사용자의 현재 상태와 변화에 따라 콘텐츠가 반응하는 것이 핵심이다. 이를 통해 교사가 긴 시간을 확보하지 못하더라도 공강이나 점심시간 중 약 10분 동안 자신만의 회복 공간에 머물 수 있는 새로운 형태의 웰니스 경험을 제안하였다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
            { label: 'Concept', body: '교사에게는 쉬는 시간이 있어도 실제로 긴장을 내려놓을 수 있는 환경이 부족하다. 수업과 생활지도 이후에도 행정업무, 학부모 연락, 동료의 시선과 교무실의 소음이 이어지기 때문에 물리적으로 앉아 있는 시간과 심리적으로 회복하는 시간 사이에는 차이가 존재한다. 기존의 휴게실이나 안마의자는 업무 환경과 완전히 분리되기 어렵고, 명상 앱은 스마트폰 알림과 다른 콘텐츠의 방해를 받기 쉬우며, 전문 상담은 일상적으로 이용하기에는 시간과 진입 부담이 크다는 한계가 있었다. 이에 Neuroscape는 회복에 필요한 것은 더 오래 쉬는 것이 아니라, 짧은 시간이라도 업무 환경과 심리적으로 분리되는 경험이라고 정의하였다. VR을 통해 외부 자극을 차단하고, EEG 신호를 콘텐츠에 연결해 사용자가 자신의 상태 변화에 자연스럽게 집중하도록 하였다. 특히 EEG 수치를 그대로 점수화해 평가하기보다, 신호의 변화가 화면 속 오브젝트와 환경의 움직임으로 나타나도록 설계하였다. 이를 통해 사용자가 자신의 상태를 분석하거나 판단해야 하는 부담 없이, 변화하는 풍경을 바라보는 과정 자체에 몰입하며 짧은 순간이라도 외부 자극에서 벗어날 수 있도록 하고자 하였다.' },
            { label: 'Process', body: '본 프로젝트에서는 서비스 기획과 사용자 경험 설계, VR 콘텐츠 구조 기획을 중심으로 프로젝트에 참여하였다. 주요 사용자인 교사의 업무 환경을 분석한 뒤, 긴 명상 프로그램보다 공강과 점심시간에도 이용 가능한 약 10분의 짧은 세션이 현실적이라고 판단하였다. 이에 VR 착용, 상태 확인, 개인 기준선 측정, 몰입 체험, 결과 확인으로 이어지는 전체 사용자 흐름을 설계하였다. 콘텐츠에서는 사용자의 EEG 신호를 단순히 측정하는 데 그치지 않고 실제 경험과 연결하는 것을 중요하게 보았다. 사용자의 초기 상태를 기준선으로 설정한 뒤, 이후 측정되는 EEG 변화에 따라 TouchDesigner 기반의 미디어아트 오브젝트가 반응하도록 구성하였다. 특히 EEG 신호의 진폭과 상태 변화에 따라 명상을 유도하는 오브젝트가 확산되거나 변화하도록 설계하여, 사용자가 자신의 내적 변화를 시각적으로 인지할 수 있도록 하였다. 또한 EEG 데이터를 그대로 노출하거나 복잡한 수치로 보여주는 대신, 입자와 빛, 색, 오브젝트의 움직임으로 변환해 감정적 부담을 낮추고 몰입을 높이고자 하였다. VR 환경에서는 사용자의 시점 이동을 최소화하고, 고정된 공간 안에서 자연스럽게 변화하는 풍경을 중심으로 구성하여 짧은 시간 안에도 안정적으로 콘텐츠를 경험할 수 있도록 하였다. 이와 함께 실제 도입 가능성을 검토하기 위해 교사와 교육기관 관계자를 대상으로 문제 상황과 이용 의향을 확인하고, 교사 개인이 사용하는 서비스이지만 기관이 비용을 지불하는 B2G 구조까지 함께 설계하였다.' },
            { label: 'Outcome', body: '최종적으로 EEG 신호와 VR 미디어아트를 연결하여, 사용자의 상태 변화에 따라 실시간으로 반응하는 10분 내외의 몰입형 회복 콘텐츠 MVP를 구현하였다. 사용자는 VR을 착용한 상태에서 초기 EEG 상태를 측정하고, 이후 자신의 변화에 따라 달라지는 미디어아트 환경을 경험할 수 있도록 구성하였다. 프로젝트 과정에서 교사와 교육기관 관계자를 대상으로 문제와 도입 가능성을 검증했으며, 자료 기준 직장 내 회복 공간 부족에 대한 문제 공감은 87%, 담당자의 예산 또는 도입 가능성 응답은 79%, 초기 체험 및 실증 참여 의향은 54.5%로 나타났다. 이를 통해 단순한 콘텐츠 제작에 그치지 않고 실제 사용 환경과 기관 도입 가능성까지 함께 검토하였다. Neuroscape를 통해 기술적으로 완성도 높은 콘텐츠를 만드는 것만으로는 충분하지 않으며, 사용자가 실제로 어떤 상황에서 이 경험을 필요로 하는지를 먼저 이해해야 한다는 점을 배웠다. 특히 EEG라는 기술을 전면에 내세우기보다, 교사가 짧은 시간 안에 자연스럽게 몰입하고 잠시 자신에게 집중할 수 있도록 기술을 경험 뒤에 배치하는 방향으로 설계하였다. 이를 통해 생체신호 데이터를 단순한 측정값이 아니라 개인의 상태와 연결된 감각적 경험으로 변환하는 가능성을 탐구하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '스타트업 경진대회' },
            { k: '사용 툴', v: 'TouchDesigner, Blender, GaussianSplatting, Cladue' },
            { k: '역할', v: '프로젝트 기획 및 인터랙티브 콘텐츠 개발' },
          ],
          media: [
        ...
        BeyondTheCenter: {
          title: 'Beyond The Center', // 🔧 최상단 화면에 크게 표시되는 대제목입니다
          cat: '제 2회 이머시브 전시회', // 🔧 제목 위나 옆에 달리는 카테고리 태그입니다
          year: '2026', // 🔧 연도 태그
          desc1: 'Beyond the Center는 포스트휴머니즘을 바탕으로 인간과 AI가 서로 연결되며 새로운 생태계를 형성하는 과정을 표현한 이머시브 미디어아트 프로젝트이다. 인간의 일상에서 발생하는 작은 행동 데이터를 하나의 ‘점’으로 시작해, 점들이 연결되어 시냅스와 도시의 네트워크를 이루고 다시 우주적 구조로 확장되는 과정을 3면 프로젝션 공간 안에 구현하였다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '작품은 LOG, WEAVE, COSMOS, LOOP의 네 단계로 구성된다. 개인의 데이터에서 시작된 연결이 점차 인간의 사고 구조와 도시 시스템, 우주적 네트워크로 확장된 뒤 다시 하나의 점으로 돌아오는 순환 구조를 통해, 인간과 기술이 분리된 존재가 아니라 서로 영향을 주고받으며 하나의 생태계를 만들어가는 관계임을 표현하고자 하였다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
            { label: 'Concept', body: 'AI는 더 이상 인간이 필요할 때만 사용하는 외부의 도구에 머물지 않고, 인간의 사고와 행동, 사회 시스템 전반에 깊게 연결되어 있다. 이에 본 프로젝트에서는 기술을 인간과 대립하는 존재로 바라보는 대신, 인간과 기술의 경계가 점차 흐려지는 포스트휴머니즘의 관점에서 미래의 관계를 바라보았다. 이를 시각적으로 표현하기 위해 인간의 일상적 행동을 데이터의 최소 단위인 ‘점’으로 설정하였다. 하나의 점은 메시지, 검색, 결제와 같은 개인의 작은 행동을 상징하고, 이러한 데이터가 서로 연결되면서 시냅스 구조를 형성한다. 이후 연결은 개인의 내부를 넘어 도시의 교통과 인프라, 다시 우주의 별자리와 궤도로 확장된다. 이처럼 동일한 ‘연결’의 구조를 개인–도시–우주로 반복 확장함으로써, 인간과 AI가 함께 만들어가는 새로운 생태계를 하나의 연속적인 이미지로 표현하였다. 마지막에는 거대하게 확장된 네트워크가 다시 하나의 점으로 돌아가며 처음과 끝을 연결한다. 이를 통해 데이터와 기술의 발전을 직선적인 진보가 아니라, 인간의 행동에서 시작되어 새로운 관계를 만들고 다시 또 다른 데이터로 이어지는 지속적인 순환으로 바라보았다.' },
            { label: 'Process', body: '본 프로젝트에서는 전시 기획과 스토리 구조 설계부터 이머시브 영상 제작 및 TouchDesigner 기반의 시각 구현 과정에 참여하였다. 먼저 추상적인 포스트휴머니즘 개념을 관객이 공간 안에서 직관적으로 경험할 수 있도록, 전체 서사를 LOG–WEAVE–COSMOS–LOOP의 네 단계로 구조화하였다. 초기 장면에서는 검은 공간에 인간의 행동 데이터를 상징하는 점과 텍스트가 생성되도록 구성하고, 이후 분리되어 있던 데이터가 서로 연결되어 네트워크를 형성하도록 구현하였다. 연결 구조는 시냅스, 도시의 데이터망, 별자리로 점차 스케일이 확장되며, 관객이 작은 데이터의 내부에서 시작해 거대한 네트워크 생태계까지 이동하는 듯한 흐름을 경험하도록 설계하였다. 최종 영상은 약 1분 35초의 순환 구조로 제작하여, 반복 상영되더라도 처음과 끝이 자연스럽게 이어지도록 구성하였다. 제작 과정에서는 TouchDesigner를 활용해 점과 선으로 이루어진 데이터 네트워크를 구현하고, AI로 생성한 시냅스·도시·우주 이미지를 영상 소스로 활용하였다. 이후 블랍 트래킹과 영상 합성을 통해 각 장면이 분리되지 않고 하나의 연결 구조로 이어지도록 구성하였다. 또한 3면 프로젝션 환경을 고려해 화면의 확장 방향과 오브젝트의 이동을 설계하여, 관객이 단순히 영상을 바라보는 것이 아니라 데이터 구조 내부에 들어온 듯한 공간감을 느낄 수 있도록 제작하였다.' },
            { label: 'Outcome', body: '최종적으로 인간의 작은 행동 데이터가 연결과 확장을 거쳐 새로운 생태계를 형성하는 과정을 3면 프로젝션 기반의 이머시브 콘텐츠로 완성하였다. 점에서 시작해 네트워크, 시냅스, 도시, 우주로 확장되는 시각적 스케일과 처음으로 되돌아오는 루프 구조를 통해 포스트휴머니즘의 순환적 관계를 공간 안에 구현하였다. 이 프로젝트를 통해 철학적 개념을 서사 구조와 시각적 메타포, 공간 연출로 전환하고, TouchDesigner와 AI 기반 영상 제작 방식을 이머시브 환경에 결합하는 과정을 경험하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '전시 프로젝트' },
            { k: '사용 툴', v: 'TouchDesigner, MadMapper' },
            { k: '역할', v: '프로젝션 매핑 및 미디어아트 제작' },
          ],
          media: [
            // 🔧 설명 섹션 옆에 순서대로 화면을 채울 이미지나 영상 소스들입니다.
            // 위에서 작성한 sections 배열 요소 순서에 맞게 하나씩 1:1로 옆자리에 배치됩니다.
        ...
        XrStudio: {
          title: '파국의 궤도', // 🔧 최상단 화면에 크게 표시되는 대제목입니다
          cat: 'XR스튜디오 쇼케이스', // 🔧 제목 위나 옆에 달리는 카테고리 태그입니다
          year: '2026', // 🔧 연도 태그
          desc1: '파국의 궤도는 단테의 『신곡』 중 애욕, 폭력, 배신의 지옥을 현대적으로 재해석한 미디어아트 작품이다. 세 개의 지옥을 관계가 무너지는 과정으로 연결하여, 욕망에서 시작된 감정이 폭력과 배신으로 이어지며 결국 파국에 이르는 흐름을 시각적으로 표현하였다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '작품의 처음과 끝에는 동일한 붉은 눈이 등장한다. 이 눈은 인간의 모든 선택과 그 결과를 지켜보는 시선이자, 지옥이 외부에서 주어진 형벌이 아니라 스스로의 선택이 만들어낸 결과임을 암시하는 상징으로 활용하였다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
            { label: 'Concept', body: '단테의 『신곡』은 죄의 종류에 따라 지옥을 구분하지만, 본 프로젝트에서는 서로 분리된 지옥을 하나의 관계가 붕괴하는 과정으로 다시 바라보았다. 관계 속 욕망과 집착을 애욕의 지옥으로, 감정이 통제되지 않고 타인을 해치는 순간을 폭력의 지옥으로, 신뢰가 완전히 무너지는 마지막 단계를 배신의 지옥으로 연결하였다. 이를 통해 세 개의 지옥을 단순히 나열하는 대신, 하나의 선택이 다음 결과를 만들고 결국 파국으로 이어지는 연속적인 서사로 구성하였다. 관객이 원작의 내용을 자세히 알지 못하더라도 색과 움직임, 분위기의 변화만으로 관계가 점차 무너지는 흐름을 직관적으로 느낄 수 있도록 하는 것이 작품의 핵심 방향이었다.' },
            { label: 'Process', body: '본 프로젝트에서는 작품의 전체 콘셉트와 시각 연출을 기획하고, TouchDesigner를 활용하여 실시간 그래픽 기반의 미디어아트 영상을 제작하였다. 먼저 애욕, 폭력, 배신의 지옥이 각각 독립적인 장면으로 보이지 않도록 세 구간의 감정적 강도와 시각적 흐름을 단계적으로 설계하였다. 애욕의 지옥에서는 끌림과 집착이 반복되는 움직임을 중심으로 표현하고, 폭력의 지옥에서는 보다 빠르고 거친 움직임과 강한 시각적 충돌을 활용해 감정의 폭발을 강조하였다. 마지막 배신의 지옥에서는 관계가 완전히 단절되고 붕괴되는 인상을 중심으로 전체 흐름을 마무리하였다. TouchDesigner에서는 오브젝트의 움직임과 형태 변화, 색과 속도의 조절을 통해 각 지옥의 감정적 특성을 구현하였다. 특히 장면별 효과를 개별적으로 만드는 데 그치지 않고, 애욕에서 폭력, 배신으로 갈수록 시각적 긴장감이 점차 고조되도록 연결하여 하나의 연속적인 경험으로 구성하는 데 중점을 두었다. 또한 작품의 시작과 끝에 붉은 눈을 반복적으로 배치해 전체 서사를 하나의 순환 구조로 완성하였다.' },
            { label: 'Outcome', body: '최종적으로 애욕, 폭력, 배신의 지옥을 관계가 파국에 이르는 하나의 과정으로 재구성한 미디어아트 작품을 완성하였다. TouchDesigner를 활용해 감정의 변화를 움직임과 형태, 색으로 구현하고, 세 장면의 긴장이 점차 강해지도록 연출하였다. 처음과 끝에 배치한 붉은 눈은 인간의 선택과 그 결과를 연결하는 상징적 구조로 기능한다. 이 프로젝트를 통해 고전 문학의 핵심 개념을 현대적인 관계의 서사로 전환하고, 이를 생성형 비주얼과 실시간 그래픽으로 표현하는 방법을 탐구하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '전시 프로젝트' },
            { k: '사용 툴', v: 'TouchDesigner, Suno' },
            { k: '역할', v: '미디어아트 기획 및 제작' },
          ],
          media: [
            // 🔧 설명 섹션 옆에 순서대로 화면을 채울 이미지나 영상 소스들입니다.
            // 위에서 작성한 sections 배열 요소 순서에 맞게 하나씩 1:1로 옆자리에 배치됩니다.
            { type: 'img', src: 'img/XrStudio.jpg', label: 'img' },
            { type: 'pdf', src: 'https://drive.google.com/file/d/1JUDAMn8yTGQ0qtuGYOLIoejFBUgbSseO/view?usp=sharing', label: 'pdf' },
            { type: 'video', src: 'https://drive.google.com/file/d/1ZbmVpZwFRlPIEw2KhcgOynlWkr4F228a/view?usp=sharing', label: 'video' },
            // { type: 'video', src: 'video/sample.mp4', label: '비디오 캡션' } // 영상을 넣고 싶으시면 이렇게 하세요!
          ]
        },
    Invader: {
          ...
          media: [
            ...
            { type: 'video', src: 'https://drive.google.com/file/d/1bVQEpydCDbwR4N0nAQGbl3Q5rHg0KKh9/view?usp=sharing', label: 'Invader Gameplay' },
          ]
        },
    JalTayo: {
          ...
          year: '2026', // 🔧 연도 태그
          desc1: '잘타요VR은 VR 기술을 활용하여 어린이들이 안전하고 올바른 대중교통 이용 방법을 체험형 학습으로 익힐 수 있도록 돕는 교육용 VR 플랫폼 프로젝트이다. 특히 실제 버스 이용 경험이 부족한 어린이들도 가상 환경 속에서 직접 버스를 기다리고, 탑승하고, 교통카드를 태그하며, 하차벨을 누르고, 안전하게 하차하는 과정을 반복적으로 체험할 수 있도록 설계된 것이 특징이다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '이 프로젝트는 기존의 설명 중심 안전교육에서 벗어나, 사용자의 행동과 인터랙션을 기반으로 학습이 이루어지는 VR 시뮬레이션 환경을 제공하는 것을 목표로 한다. 또한 어린이 친화적인 디자인과 직관적인 튜토리얼 구조를 통해 대중교통 이용에 대한 두려움을 줄이고, 실제 상황에 가까운 체험 학습을 통해 안전한 이동 습관 형성에 도움을 주는 새로운 형태의 실감형 교육 콘텐츠를 제안한다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
            { label: 'Concept', body: '최근 맞벌이 가정 증가와 생활 환경 변화로 인해 어린이들이 혼자 대중교통을 이용해야 하는 상황이 늘어나고 있다. 하지만 기존의 대중교통 안전교육은 영상 시청이나 이론 중심으로 진행되는 경우가 많아 실제 상황에서 필요한 행동을 충분히 익히기 어렵다는 한계가 존재한다. 특히 버스 탑승 순서, 교통카드 태그, 하차벨 사용과 같은 행동은 직접 경험해보지 않으면 익숙해지기 어렵기 때문에 어린이들이 실제 대중교통 이용 과정에서 불안감을 느끼는 경우가 많다. 잘타요VR은 이러한 문제를 해결하기 위해 VR 기반의 체험형 대중교통 교육 시스템을 제안한다. 사용자는 가상 환경 속에서 실제와 유사한 버스 이용 과정을 직접 경험하며 자연스럽게 대중교통 이용 방법을 학습할 수 있다. 또한 단순 시청형 교육이 아닌 행동 기반 인터랙션 구조를 통해 어린이들이 능동적으로 참여할 수 있도록 설계하였으며, 반복 학습이 가능한 VR 환경을 통해 안전한 대중교통 이용 습관 형성을 돕는 것을 목표로 한다.' },
            { label: 'Process', body: '본 프로젝트에서는 기획과 유니티 VR 개발을 담당하여 전체 서비스 구조와 사용자 경험을 설계하고 구현하였다. 주요 타깃이 어린이 사용자라는 점을 고려하여 복잡한 조작보다는 직관적이고 쉽게 이해할 수 있는 학습 흐름을 만드는 것을 중요하게 생각하였다. 이를 위해 실제 버스 이용 과정과 동일한 순서로 튜토리얼 구조를 설계하여, 사용자가 버스를 기다리고 탑승한 뒤 교통카드를 태그하고 하차벨을 누른 후 안전하게 하차하는 과정을 자연스럽게 익힐 수 있도록 구성하였다. 또한 VR 환경에서 어린이들이 쉽게 몰입할 수 있도록 인터랙션 중심의 콘텐츠를 구현하였으며, XR Interaction Toolkit과 Unity를 활용하여 카드 태그, 하차벨, 좌석 탑승 등 실제 행동 기반의 상호작용 시스템을 개발하였다. 특히 사용자의 행동에 따라 버스가 정차하거나 이동하는 시스템을 구현하여 단순 체험형 콘텐츠가 아닌 실제 상황과 유사한 학습 경험을 제공하고자 하였다. 이외에도 어린이 친화적인 UI와 시각 요소를 적용하여 사용자가 부담 없이 학습할 수 있는 VR 교육 환경을 제작하는 데 중점을 두었다.' },
            { label: 'Outcome', body: '최종적으로 버스 대기부터 탑승, 교통카드 태그, 하차벨 사용과 하차까지 이어지는 과정을 VR 인터랙션으로 구현한 체험형 교육 콘텐츠를 완성하였다. Unity와 XR Interaction Toolkit을 활용해 실제 행동에 기반한 상호작용 시스템을 제작하고, 어린이 친화적인 UI와 튜토리얼을 적용해 처음 VR을 접하는 사용자도 흐름을 쉽게 따라가도록 구성하였다. 이를 통해 설명 중심 안전교육을 보완할 수 있는 반복 체험형 대중교통 교육 콘텐츠의 가능성을 탐구하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '개발자 경진대회' },
            { k: '사용 툴', v: 'Unity, Blender' },
            { k: '역할', v: '프로젝트 기획 및 VR 개발' },
          ],
          media: [
            // 🔧 설명 섹션 옆에 순서대로 화면을 채울 이미지나 영상 소스들입니다.
            // 위에서 작성한 sections 배열 요소 순서에 맞게 하나씩 1:1로 옆자리에 배치됩니다.
            { type: 'img', src: 'img/JalTayo.png', label: 'img' },
    ...
    PiratesStorm: {
          ...
          media: [
            ...
            { type: 'video', src: 'https://drive.google.com/file/d/1OhU5AiFPJhbOyiOw62D7QFOpNWXq_nb6/view?usp=sharing', label: 'PiratesStorm Gameplay' },
          ]
        },
        TeamPL: {
          title: '팀플!', // 🔧 최상단 화면에 크게 표시되는 대제목입니다
          cat: '제 3회 COSS 스타트업 경진대회', // 🔧 제목 위나 옆에 달리는 카테고리 태그입니다
          year: '2026', // 🔧 연도 태그
          desc1: '대학생 팀 프로젝트에서 발생하는 회의 불참, 정보 누락, 업무 혼선을 해결하기 위한 AI 기반 팀플 운영 플랫폼이다. 대학생 팀플에서는 수업 시간표, 아르바이트, 개인 일정 등으로 인해 모든 팀원이 매번 회의에 참여하기 어렵다. 문제는 단순히 회의에 빠지는 것이 아니라, 회의 이후 공유되는 회의록만으로는 논의의 흐름과 결정의 이유를 충분히 이해하기 어렵다는 점이다.', // 🔧 최상단 첫번째 요약 설명
          desc2: '팀플!은 이 문제를 해결하기 위해 회의 내용을 AI가 분석하고, 단순 요약이 아닌 회의 맥락 복원, 핵심 결정 정리, 업무 카드화, 불참자용 따라잡기 경험을 제공하는 서비스로 기획했다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            // 🔧 스크롤해서 내릴 때 사진 옆에 나타나는 설명 텍스트들입니다. 
            // 원하는 개수만큼 { label, body } 세트를 늘리면 자동으로 박스가 계속 생겨납니다.
        { label: 'Concept', body: '팀플!의 핵심 콘셉트는 “회의록이 아니라 회의 맥락을 복원하는 것”이다. 기존 회의 요약 서비스가 회의 내용을 텍스트로 정리하는 데 집중했다면, 팀플!은 회의에서 나온 발언의 배경, 논의가 전환된 이유, 최종 결정이 형성되는 과정을 함께 보여주는 데 초점을 둔다. 예를 들어 회의에 참석하지 못한 사용자가 플랫폼에 접속하면, AI가 정리한 회의 흐름을 기반으로 팀원 캐릭터들이 대화하듯 회의 내용을 재현한다. 사용자는 단순한 요약문을 읽는 것이 아니라, 실제 회의가 어떤 순서로 전개되었는지 시각적으로 따라갈 수 있다. 이를 통해 회의 참여자와 불참자 사이의 정보 격차를 줄이고, 팀 전체가 같은 맥락 위에서 다음 작업을 이어갈 수 있도록 한다. 팀플!은 회의 내용을 정리하는 것을 넘어, 팀이 논의하고 결정하고 실행하는 전 과정을 하나의 흐름으로 연결하는 대학생 팀 프로젝트용 AI 협업 운영 OS를 지향한다.' },
            { label: 'Process', body: '프로젝트는 먼저 대학생 팀플에서 반복적으로 발생하는 문제를 정의하는 것에서 시작했다. 초기에는 “회의에 빠진 팀원이 내용을 잘 모른다”는 단순한 상황에서 출발했지만, 문제를 구체화하면서 핵심은 회의록 부족이 아니라 회의의 맥락과 결정 과정이 전달되지 않는 데 있다는 점을 도출했다. 이에 따라 프로젝트의 방향을 단순 회의 요약 서비스가 아닌, 불참자가 회의의 흐름을 빠르게 따라잡을 수 있는 맥락 복원형 플랫폼으로 설정했다. 이후 대표 사용자 시나리오를 설계했다. 사용자는 개인 일정으로 지난 회의에 참석하지 못했고, 팀원들이 공유한 자료만으로는 왜 특정 방향으로 결정이 내려졌는지 이해하지 못하는 상황에 놓인다. 이 시나리오를 바탕으로 사용자가 필요로 하는 기능을 정리했고, 회의 흐름 요약, 캐릭터 기반 맥락 리플레이, 업무 카드 자동 생성, AI 질문 기능을 핵심 기능으로 구성했다. 스타트업 경진대회 시연을 고려해 MVP는 실제 외부 툴 연동보다 웹 기반 데모 화면 중심으로 기획했다. 랜딩페이지, 회의 업로드 화면, AI 분석 화면, 회의 흐름 요약 화면, 맥락 리플레이 화면, 업무 카드 화면으로 구성해 심사위원이 서비스의 문제와 해결 방식을 직관적으로 이해할 수 있도록 설계했다. 또한 서비스의 이미지를 명확히 전달하기 위해 3D 캐릭터 기반 원격 회의 비주얼과 스타트업 랜딩페이지 형식의 웹사이트를 함께 제작했다.' },
            { label: 'Outcome', body: '팀플! 프로젝트를 통해 대학생 팀 프로젝트에서 발생하는 회의 불참자의 맥락 단절 문제를 AI 기반 협업 운영 서비스로 구체화했다. 단순 아이디어 수준에 머무르지 않고 문제 정의, 사용자 시나리오, 핵심 기능, MVP 구조, 웹사이트 시안, 수익모델까지 포함한 스타트업형 서비스 기획으로 발전시켰다. 최종 결과물은 총 4개의 핵심 기능과 6개의 MVP 화면 구조로 정리되었다. 핵심 기능은 AI 회의 흐름 요약, 캐릭터 기반 맥락 리플레이, 업무 카드 자동 생성, AI 질문 기능으로 구성되었으며, MVP 화면은 랜딩페이지, 회의 업로드, AI 분석, 회의 요약, 맥락 리플레이, 업무 카드 화면으로 설계되었다. 서비스 경험의 목표는 회의 영상을 처음부터 끝까지 다시 보는 대신, 약 5분 안에 지난 회의의 핵심 흐름과 본인의 역할을 파악할 수 있도록 하는 것이다. 팀플!은 초기에는 대학생 팀 프로젝트를 주요 타깃으로 시작하지만, 이후 캡스톤디자인, 창업 수업, 공모전 팀, 해커톤 운영 기관, 초기 스타트업 팀까지 확장할 수 있는 가능성을 가진다. 이 프로젝트는 팀플에서 발생하는 비효율을 단순히 기록의 문제로 보지 않고, 팀원 간 맥락 공유의 문제로 재정의했다는 점에서 의미가 있다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '스타트업 경진대회' },
            { k: '사용 툴', v: 'ChatGPT, Figma, Stitch' },
            { k: '역할', v: '프로젝트 기획 및 발표 자료 제작' },
          ],
          media: [
            // 🔧 설명 섹션 옆에 순서대로 화면을 채울 이미지나 영상 소스들입니다.
            // 위에서 작성한 sections 배열 요소 순서에 맞게 하나씩 1:1로 옆자리에 배치됩니다.
            { type: 'img', src: 'img/TeamPL.png', label: 'img' },
            { type: 'pdf', src: 'https://drive.google.com/file/d/1ANKfQOK8b7UknGUftKsOjGHvtPV-BdBH/view?usp=sharing', label: 'pdf' },
            { type: 'web', src: 'https://clinquant-phoenix-3354a0.netlify.app/', label: 'web' },
          ]
        },
    dndn: {
      ...
      sections: [
        ...
        { label: 'Process', body: '본 프로젝트에 외부 참여자로 함께하며 UI/UX 기획과 디자인을 담당하여 서비스 구조와 사용자 경험을 설계하였다. 주요 타깃인 중장년층 사용자를 고려해 복잡한 기능보다 직관적인 사용 흐름에 중점을 두었다. 전체 앱을 단순한 단계로 구성하고, AI 기능을 처음 접하는 사용자도 콘텐츠 제작 과정을 쉽게 따라갈 수 있도록 앱 플로우와 인터페이스를 설계하였다. 특히 생성형 AI가 영상을 제작하는 대기 시간에 사용자가 이탈하지 않도록, 진행 상태를 자연스럽게 전달하는 로딩 페이지와 인터랙션을 구성하였다.' },
    ...
    CITY: {
      ...
      sections: [
        ...
        { label: 'Process', body: '미국 대학생들이 진행하던 프로젝트에 한국 팀원으로 합류하여 아이디어와 방향을 함께 구체화하였다. 초기 아이디어를 바탕으로 서비스의 세부 기획을 맡아 플랫폼의 구조와 기능을 정리하고, 시민 참여 흐름을 중심으로 사용자 경험을 설계하였다. 팀원들이 같은 방향을 공유할 수 있도록 플랫폼 구조와 사용자 흐름을 시각화해 데모로 구현하였다. 이후 현지 필드 리서치에서 시민들에게 플랫폼 콘셉트를 설명하고 피드백을 수집했으며, 이를 반영해 서비스 구조와 기능을 보완하였다.' },
    ...
    Jeonger: {
      ...
      sections: [
        ...
        { label: 'Process', body: 'Think City 2026 Hackathon의 제한된 시간 안에 한국과 미국 팀원들이 브레인스토밍을 진행하며 두 사회에 공통으로 나타나는 지역사회 문제와 해결 방향을 정의하였다. 프로젝트에서는 서비스 기획을 담당하여 플랫폼의 핵심 구조와 사용자 흐름을 정리하고, 발표 문서를 제작해 핵심 메시지와 서비스 구조가 명확하게 전달되도록 구성하였다. 언어와 문화적 차이가 있는 국제 협업 환경에서도 지속적으로 의견을 조율하며 아이디어를 구체화하고 최종 발표까지 완성하였다.' },
    ...
    sds: {
      ...
      sections: [
        ...
        { label: 'Outcome', body: '최종적으로 스위스 생활 정보, 언어 교류, 커뮤니티, 생활 마켓을 하나의 흐름으로 연결한 ‘스대살’의 웹 UI 디자인을 완성하였다. 해외 유학생이 겪는 정보 부족 문제를 기능 구조로 구체화하고, 이를 일관된 디자인 시스템과 사용자 중심의 웹 인터페이스로 구현하였다.' },
      ],
      details: [
        { k: '프로젝트 유형', v: '웹 디자인' },
        { k: '사용 도구', v: 'Figma' },
    ...
    supporters: {
      ...
      sections: [
        { label: 'Overview', body: '본 활동에서는 e-sports와 게임 문화 공간을 소개하는 카드뉴스와 롱폼 영상 콘텐츠를 제작하였다. 카드뉴스는 SNS에서 핵심 정보를 빠르게 파악할 수 있도록 시각적으로 정리하고, 영상은 공간의 분위기와 체험 요소가 생생하게 전달되도록 촬영과 편집을 진행하였다. 같은 공간 정보도 플랫폼과 형식에 따라 전달 방식과 스토리 구성을 달리하는 데 중점을 두었다.' },
    ...
    learncation: {
      ...
      sections: [
        ...
        { label: 'Process', body: '본 프로젝트에서는 팀원들과 함께 제주 지역 환경 문제와 관광 산업의 관계를 분석하며 프로젝트를 기획하였다. 먼저 ESG 관점에서 제주 관광의 문제점을 분석하고, 관광객이 자연스럽게 환경 보호 활동에 참여할 수 있는 체험 프로그램을 설계하였다. 이 과정에서 Needs/Wants, 3C, SWOT 분석을 통해 방향성을 정리하고, 제주의 환경·문화 자원을 활용한 프로그램을 구체화하였다. 해양 환경 체험, 지역 문화 체험, 자연 탐방을 결합하여 참여자가 단순한 관광객을 넘어 환경 보호 활동에 함께하는 체험형 관광 구조를 설계하였다.' },
      ],
      details: [
        { k: '프로젝트 유형', v: '해커톤' },
        { k: '사용 도구', v: 'Figma' },
    ...
    hana: {
      ...
      sections: [
        ...
        { label: 'Process', body: '카드 디자인 경험은 취향을 채우고(Fill), 감정을 표현한 뒤(Feel), 완성된 디자인을 브랜드 경험으로 연결하는 3단계로 구성하였다. 다양한 그래픽 요소와 컬러로 사용자의 취향과 라이프스타일을 드러내고, 이를 일관된 디자인 시스템으로 정리하였다. 또한 #EC91FA, #E9FC88, #05AA82 컬러 팔레트를 사용하여 Young Premium User를 위한 감각적이고 개성 있는 시각 스타일을 구현하였다.' },
      ],
      details: [
        { k: '프로젝트 유형', v: '공모전' },
    ...
    climate: {
      ...
      sections: [
        ...
        { label: 'Outcome', body: '최종적으로 3D 카드 오브젝트와 2D 그래픽, 해치 캐릭터의 움직임을 결합한 기후동행카드 광고 영상을 완성하였다. 기존 카드에서 새로운 디자인으로 변화하는 스토리를 통해 브랜드 메시지를 시각화하고, 친근한 캐릭터와 생동감 있는 모션으로 정책 카드의 이미지를 재해석하였다.' },
      ],
      details: [
        { k: '프로젝트 유형', v: '3D모션그래픽 광고' },
    ...
    naver: {
      ...
      sections: [
        ...
        { label: 'Outcome', body: '최종적으로 사용자가 목적지를 찾고 이동을 계획하는 흐름에 네이버 지도의 주요 기능을 결합한 광고 영상을 완성하였다. 실제 촬영 장면 위에 2D 그래픽을 더해 서비스 사용 상황을 직관적으로 표현했으며, 영상 기획부터 촬영과 편집까지 전체 제작 과정을 수행하였다.' },
      ],
      details: [
        { k: '프로젝트 유형', v: '광고 영상' },
    ...
    dmd: {
      ...
      cat: '기획기초',
      year: '2025',
      desc1: '‘디지털미디어디자인과, 우주를 닮다’는 디지털미디어디자인과 학생들의 성향을 조사하고, 그 결과를 시각적 스토리로 표현한 모션 영상 프로젝트이다. 설문에 나타난 학생들의 특성을 MBTI 요소로 분석한 결과 INTP 성향과 유사한 경향을 발견하였고, 이를 영상 콘텐츠로 제작하였다.',
      desc2: '제작된 영상은 디지털 사이니지 환경을 고려하여 제작되었으며, X-Space LED 월에 상영되는 것을 목표로 한 대형 화면 콘텐츠로 기획되었다.',
      sections: [
        { label: 'Overview', body: '설문 결과에서는 디지털미디어디자인과 학생들에게 시각적 감각과 창의성, 새로운 미디어 표현 방식과 기술 융합에 대한 관심이 나타났다. 또한 디자인 작업에서 논리적인 사고와 실험적인 접근을 함께 활용하는 경향을 확인하였다. 이 특성을 흥미롭게 전달하기 위해 MBTI 요소를 바탕으로 학과의 성향을 시각적으로 해석하고, INTP와 유사한 특징을 ‘우주’라는 메타포로 연결하였다. 깊은 사고, 넓은 시야, 논리적 구조, 유연한 사고를 우주의 이미지로 시각화하는 것이 콘텐츠의 핵심이다.' },
  ...
  function buildProjectPageHTML(data, isDark) {
    ...
    const MANUAL_BREAKS = {
      '연결': [[2, 5], [2, 5, 7], [1, 3]],
      Neuroscape: [[2, 4], [2, 5, 7], [2, 5]],
      'Beyond The Center': [[2, 5], [2, 5], [2]],
      '파국의 궤도': [[2], [2, 4], [2]],
      '잘타요VR': [[2, 4], [2, 4], [2]],
      '팀플!': [[2, 5, 8], [3, 6], [2, 5, 7]],
      '든든AI': [[2], [2, 4]],
      ...
      '토스증권 광고': [[2], [2, 4], [1]],
      스대살: [[2], [2, 4]],
      '슈퍼플레이 서포터즈 2기': [[1], [2], [2], [1]],
      '가치 제주, 고치 제주': [[2, 4], [2]],
      'Fill The [ ], Feel MY [ ]': [[2], [2, 4]],
      '기후동행카드 광고': [[2], [2, 4], [1]],
      '네이버지도 광고': [[2], [2, 4], [1]],
      '디미디, 우주를 닮다': [[2, 4], [2, 4], [1]]
    };
    const MANUAL_HERO_BREAKS = {
      '팀플!': [[1], []],
      '든든AI': [[2], []],
      CITY: [[1], []],
      Jeonger: [[2], []],
    ...
    function formatReadableText(text, breaks = []) {
      const value = String(text || '').trim();
      const sentences = value.match(/.*?[.!?]+(?:['”’)]*)?(?=\s|$)|.+$/g);
      if (!sentences || !breaks.length) return value;
      return sentences.map((sentence, index) =>
        sentence.trim() + (breaks.includes(index + 1) ? '<br><br>' : ' ')
      ).join('').trim();
    }
    ...
    const scrollSectionsHTML = sections.map((sec, i) => {
      ...
      <p class="ss-body">${formatReadableText(sec.body, (MANUAL_BREAKS[data.title] || [])[i])}</p>
    </div>`;
...
<link rel="stylesheet" href="project-cursor.css">
<style>
/* ── CSS 변수 ── */
:root {
  --bg:   ${bg};
...
<div class="proj-hero ss-reveal">
  ...
  <div class="hero-desc">
    <p>${formatReadableText(data.desc1, (MANUAL_HERO_BREAKS[data.title] || [])[0])}</p>
    ${data.desc2 ? `<p>${formatReadableText(data.desc2, (MANUAL_HERO_BREAKS[data.title] || [])[1])}</p>` : ''}
  </div>
</div>
...
<\/script>
<script src="project-cursor.js"><\/script>
</body>
</html>`;
  }
