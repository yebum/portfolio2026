...
(function () {
  ...
  const PROJECTS = {
    BioAx: {
          ...
          desc2: '서비스는 AI 바이오 코스 추천, 이동 경로 자체를 하나의 그림으로 완성하는 드로잉 코스, 방문 횟수에 따라 혜택이 확장되는 단계형 바우처로 구성된다. 관광객에게는 새로운 로컬 여행 경험을 제공하고, 바이오 기업과 제주도에는 방문·완주·구매 행동 데이터를 제공함으로써 관광 경험이 산업의 소비와 성과로 이어지는 구조를 제안하였다.', // 🔧 최상단 두번째 요약 설명
          sections: [
            ...
            { label: 'Outcome', body: '최종적으로 홈, 여행 조건 입력, AI 코스 추천, 지도 기반 투어, 드로잉 코스와 바우처로 이어지는 모바일 서비스 MVP를 완성하였다. 관광객은 자신의 조건에 맞는 코스를 추천받고, 제주 곳곳의 바이오 관련 장소를 하나의 이야기와 경로로 경험하며, 방문과 완주 결과를 기록으로 남길 수 있도록 구성하였다. 또한 사용자 경험에서 발생하는 코스 선택, POI 방문, 완주, 쿠폰 사용 데이터를 기업과 운영자가 확인할 수 있는 대시보드 구조를 제안하였다. 이를 통해 바이오 기업은 어떤 원료와 제품이 관광객의 관심과 구매로 이어지는지 확인하고, 제주도는 지역별 방문과 소비 전환을 기반으로 관광·바이오 정책의 성과를 측정할 수 있도록 하였다. 이 프로젝트를 통해 공공데이터 분석에서 발견한 문제를 실제 사용자 경험과 서비스 구조로 전환하고, 다시 사용자의 행동 데이터를 산업의 성과로 연결하는 전 과정을 경험하였다. 특히 기술이나 데이터를 독립적인 기능으로 제시하기보다, 관광객에게는 재미있는 여행 경험으로 보이면서 동시에 기업과 지역에는 측정 가능한 가치를 제공하는 다면적 서비스 구조를 설계했다는 점에서 의미가 있었다.' }
          ],
          details: [
            { k: '프로젝트 유형', v: '해커톤' },
        ...
        BeyondTheCenter: {
          ...
          sections: [
            ...
            { label: 'Outcome', body: '최종적으로 인간의 작은 행동 데이터가 연결과 확장을 거쳐 새로운 생태계를 형성하는 과정을 3면 프로젝션 기반의 이머시브 콘텐츠로 완성하였다. 점, 네트워크, 시냅스, 도시, 우주로 이어지는 시각적 스케일의 변화를 통해 포스트휴머니즘이라는 추상적인 개념을 관객이 하나의 공간적 흐름으로 경험할 수 있도록 하였다. 특히 작품의 시작과 끝을 하나의 점으로 연결한 루프 구조를 통해 인간과 AI의 관계를 완결된 결과가 아니라 지속적으로 생성되고 변화하는 생태계로 표현하였다. 이 프로젝트를 통해 철학적 개념을 단순한 설명에 머물게 하지 않고, 서사 구조와 시각적 메타포, 공간 연출로 변환하는 과정을 경험했으며, TouchDesigner와 AI 기반 영상 제작 방식을 결합해 이머시브 환경에 적합한 콘텐츠를 구현하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '전시 프로젝트' },
    ...
    XrStudio: {
          ...
          sections: [
            ...
            { label: 'Outcome', body: '최종적으로 애욕, 폭력, 배신의 지옥을 관계가 파국에 이르는 하나의 과정으로 재구성한 미디어아트 작품을 완성하였다. TouchDesigner를 활용해 각 단계의 감정 변화를 움직임과 형태, 색의 변화로 구현하였으며, 세 개의 장면이 점차 강해지는 하나의 흐름으로 이어지도록 연출하였다. 특히 처음과 끝에 동일한 붉은 눈을 등장시켜 작품 전체에 하나의 상징적 구조를 부여하였다. 이를 통해 관객이 지옥을 단순한 형벌의 공간으로 바라보는 것이 아니라, 인간의 선택과 관계가 만들어낸 결과로 해석할 수 있도록 하였다. 이 프로젝트를 통해 고전 문학의 세계관을 그대로 재현하기보다, 핵심 개념을 현대적인 관계의 서사로 변환하고 이를 생성형 비주얼과 실시간 그래픽으로 표현하는 방법을 탐구하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
            { k: '프로젝트 유형', v: '전시 프로젝트' },
    ...
    JalTayo: {
          ...
          sections: [
            ...
            { label: 'Outcome', body: '최종적으로 VR 환경에서 어린이들이 직접 대중교통 이용 과정을 체험하며 학습할 수 있는 체험형 교육 콘텐츠를 완성하였다. 잘타요VR은 버스 대기, 탑승, 교통카드 태그, 하차벨 사용, 하차까지 이어지는 실제 대중교통 이용 흐름을 VR 인터랙션 기반으로 구현하여, 사용자가 단순히 보는 교육이 아닌 직접 행동하며 익히는 학습 경험을 제공한다. 또한 Unity와 XR Interaction Toolkit을 활용하여 실제 상황과 유사한 상호작용 시스템을 구현하였으며, 어린이 친화적인 UI와 튜토리얼 구조를 통해 처음 VR을 접하는 사용자들도 쉽게 이용할 수 있는 환경을 제시하였다. 이를 통해 기존 이론 중심 안전교육의 한계를 보완하고, 반복 체험 기반의 실감형 대중교통 교육 콘텐츠 가능성을 탐구하는 프로젝트로 발전하였다.' }
          ],
          details: [
            // 🔧 페이지 맨 밑에 들어가는 표 형식의 구체적인 스펙 정보입니다.
